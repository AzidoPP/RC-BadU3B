...
#define SHIFT 0x80
#define FAST_PRINT_MAX_KEYS 6
#define FAST_PRINT_LSHIFT 0x02
...
uint8_t USB_EP1_send() {
  ...
  while (UpPoint1_Busy) {
    waitWriteCount++;
    if (KEYBOARD_EP1_SEND_WAIT_US > 0) {
      delayMicroseconds(KEYBOARD_EP1_SEND_WAIT_US);
    }
    if (waitWriteCount >= KEYBOARD_EP1_SEND_TIMEOUT_US)
      return 0;
  }
...
static bool Keyboard_fastprint_contains_key(
    __data uint8_t keys[FAST_PRINT_MAX_KEYS], __data uint8_t len,
    __data uint8_t key) {
...
static uint8_t Keyboard_fastprint_send(__data uint8_t modifier,
                                       __data uint8_t keys[FAST_PRINT_MAX_KEYS],
                                       __data uint8_t len) {
  __data uint8_t pressOk;
  __data uint8_t releaseOk;
  ...
  HIDKey[0] = modifier;
  for (__data uint8_t i = 0; i < len && i < FAST_PRINT_MAX_KEYS; i++) {
    HIDKey[2 + i] = keys[i];
  }

  pressOk = USB_EP1_send();

  for (__data uint8_t i = 0; i < sizeof(HIDKey); i++) {
    HIDKey[i] = 0;
  }

  releaseOk = USB_EP1_send();
  return pressOk && releaseOk;
}

static uint8_t Keyboard_fastprint_flush(
    __data uint8_t keys[FAST_PRINT_MAX_KEYS], __data uint8_t *len,
    __data bool needsShift) {
  __data uint8_t result = 1;

  if (*len == 0) {
    return 1;
  }

  result = Keyboard_fastprint_send(needsShift ? FAST_PRINT_LSHIFT : 0, keys,
                                   *len);
  for (__data uint8_t i = 0; i < FAST_PRINT_MAX_KEYS; i++) {
    keys[i] = 0;
  }
  *len = 0;
...
uint8_t Keyboard_fastprint(const char *str) {
  __data uint8_t keys[FAST_PRINT_MAX_KEYS] = {0};
  __data uint8_t len = 0;
  __data bool segmentNeedsShift = false;
  __data uint8_t result = 1;
  __data uint8_t c;

  while ((c = *str++)) {
    __data uint8_t mapped;
    __data uint8_t hidKey;
    __data bool needsShift;
    __data bool mustSplit = false;

    if (c == '\n' || c == '\r' || c == '\t' || c == '\b') {
      __data uint8_t specialLen = 1;
      if (c == '\r') {
        hidKey = 0x28;
      } else {
        hidKey = _asciimap[c];
      }
      result &= Keyboard_fastprint_flush(keys, &len, segmentNeedsShift);
      segmentNeedsShift = false;
      keys[0] = hidKey;
      result &= Keyboard_fastprint_flush(keys, &specialLen, false);
      keys[0] = 0;
      continue;
    }
    ...
    hidKey = mapped & 0x7f;
    needsShift = (mapped & SHIFT) != 0;

    if (len >= FAST_PRINT_MAX_KEYS) {
      mustSplit = true;
    } else if (len > 0 && needsShift != segmentNeedsShift) {
      mustSplit = true;
    } else if (Keyboard_fastprint_contains_key(keys, len, hidKey)) {
      mustSplit = true;
    }
